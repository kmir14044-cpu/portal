...
function renderZiyarats() {
  $("#ziyaratTable").innerHTML = getZiyarats().map((item) => `<tr>
    <td data-label="Image"><img class="tableThumb" src="${escapeHtml(item.image || './transport-rate-list.jpeg')}" alt="${escapeHtml(item.name)}"></td>
    <td data-label="Name"><b>${escapeHtml(item.name)}</b></td><td data-label="Price">SAR ${Number(item.price || 0).toLocaleString()}</td>
    <td class="actionCell" data-label="Action"><div class="rowActions leadActions"><button data-view-ziyarat="${item.id}" type="button">View</button><button data-edit-ziyarat="${item.id}" type="button">Edit</button><button class="danger" data-delete-ziyarat="${item.id}" type="button">Delete</button></div></td>
  </tr>`).join("");
  $$('[data-view-ziyarat]').forEach(b=>b.addEventListener('click',()=>viewRecord('Ziyarat Details',getZiyarats().find(x=>x.id===b.dataset.viewZiyarat))));
  $$('[data-edit-ziyarat]').forEach(b=>b.addEventListener('click',()=>openZiyaratEditor(getZiyarats().find(x=>x.id===b.dataset.editZiyarat))));
  $$('[data-delete-ziyarat]').forEach(b=>b.addEventListener('click',()=>deleteZiyarat(b.dataset.deleteZiyarat)));
}
...
function openZiyaratEditor(item) {
  recordMode = "ziyarat";
  const record=item||{id:`ziyarat-${Date.now()}`,name:"",price:0,image:""}; recordId=record.id;
  $("#recordTitle").textContent=item?"Edit Ziyarat":"Add Ziyarat";
  $("#recordFields").innerHTML=`
    <label>Name<input id="ziyaratName" value="${escapeHtml(record.name)}" required></label>
    <label>Price SAR<input id="ziyaratPrice" type="number" min="0" value="${Number(record.price)||0}" required></label>
    <label class="fullField">Image URL<input id="ziyaratImage" value="${escapeHtml(record.image||'')}" placeholder="https://..."></label>
...
function saveRecord(event) {
  ...
  if (recordMode === "ziyarat") {
    const overrides=getZiyaratOverrides(); overrides[recordId]={id:recordId,name:$("#ziyaratName").value.trim(),price:Number($("#ziyaratPrice").value)||0,image:$("#ziyaratImage").value.trim()}; storageSet("umrahZiyaratOverrides",overrides);
  }
...
function deleteZiyarat(id) { if (!confirm("Delete this Ziyarat?")) return; storageSet("umrahDeletedZiyarats", [...new Set([...getDeletedZiyarats(), id])]); renderAll(); }
...
$("#addZiyaratBtn").addEventListener("click", () => openZiyaratEditor(null));
