...
function selectedZiyaratRecords() {
  return activeZiyarats().filter((item) => state.selectedZiyarats.includes(item.id));
}

function ziyaratRateSar() {
  return selectedZiyaratRecords().reduce((sum, item) => sum + (Number(item.price) || 0), 0);
}

function ziyaratLabel() {
  return selectedZiyaratRecords().map((item) => item.name).join(", ") || "Ziyarat";
}
...
function routeCards() {
  ...
  return `<div class="routeCards">${options.map(([id, label, hint]) => `
    <button type="button" class="routeCard ${state.routePresetId === id ? "active" : ""}" data-route-card="${id}">
      <span class="routeThumb routeThumb-${id}" aria-hidden="true"></span>
      <span class="routeText"><b>${label}</b><small>${hint}</small></span>
      <span class="routeCheck" aria-hidden="true"></span>
    </button>
  `).join("")}</div>`;
}
...
function reviewItineraryBlock() {
  const q = quote();
  const ziyaratRows = state.ziyarat ? selectedZiyaratRecords().map((item) => `
    <div class="reviewDay"><b>${displayText(item.name)}</b><small>Guided Ziyarat tour - SAR ${Number(item.price || 0).toLocaleString()}</small></div>
  `).join("") : "";
...
function extrasField() {
  const ziyarats = activeZiyarats();
  return `<div class="miniPanel extrasPanel"><h3>Extras</h3>
    ${toggle("Ziyarat Tours", "ziyarat", "Guided holy site visits")}
    ${state.ziyarat ? `<div class="field big ziyaratSection" aria-live="polite">
      <span>Ziyarat Tours</span>
      <div class="ziyaratList">
        ${ziyarats.length ? ziyarats.map((item) => `<label class="ziyaratChoice ${state.selectedZiyarats.includes(item.id) ? "active" : ""}">
          <input type="checkbox" data-ziyarat-id="${item.id}" ${state.selectedZiyarats.includes(item.id) ? "checked" : ""}>
          <img src="${item.image || './transport-rate-list.jpeg'}" alt="${item.name}" onerror="this.onerror=null;this.src='./transport-rate-list.jpeg';">
          <span><b>${item.name}</b><small>SAR ${Number(item.price || 0).toLocaleString()}</small></span>
        </label>`).join("") : `<div class="emptyZiyarat">No Ziyarat tours are available.</div>`}
      </div>
    </div>` : ""}
  </div>`;
}
...
function commitField(input) {
  ...
  if (key === "ziyarat") {
    state.ziyarat = Boolean(value);
    if (!state.selectedZiyarats.length && activeZiyarats()[0]) state.selectedZiyarats = [activeZiyarats()[0].id];
    render();
    return;
  }
  if (key === "ziyaratType") {
    state.ziyaratType = value || "taif";
    render();
    return;
  }
...
function bind() {
  ...
  document.querySelectorAll("[data-ziyarat-id]").forEach((input) => input.addEventListener("change", () => {
    const id = input.dataset.ziyaratId;
    state.selectedZiyarats = input.checked ? [...new Set([...state.selectedZiyarats, id])] : state.selectedZiyarats.filter((item) => item !== id);
    render();
  }));
...
function renderItinerary(q = quote()) {
  ...
  document.getElementById("itineraryList").innerHTML = `
    ...
    ${state.ziyarat ? selectedZiyaratRecords().map((item) => `<article class="day ziyaratDay">
      <img class="photo" src="${item.image || './transport-rate-list.jpeg'}" alt="${displayText(item.name)}">
      <div class="timeline">
        <h3>${displayText(item.name)}</h3>
        <ul>
          <li>Guided Ziyarat tour included in your package</li>
          <li>Tour price: SAR ${Number(item.price || 0).toLocaleString()}</li>
          <li>Exact tour date and pickup time will be confirmed with the final booking</li>
        </ul>
      </div>
    </article>`).join("") : ""}
  `;
